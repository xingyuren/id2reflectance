...
def cosin_loss(x1, x2):
    # return np.dot(x1, x2) / (np.linalg.norm(x1) * np.linalg.norm(x2))
    return (1 - torch.sum(x1 * x2, dim=1)).mean()
    # return (1 - torch.sum(x1 * x2, dim=1) / (torch.norm(x1, dim=1) * torch.norm(x2, dim=1))).mean()
...
class CosineLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        """
        return self.loss_weight * cosin_loss(pred, target)
...
class LPIPSLoss(nn.Module):
    ...
    def forward(self, pred, target, normalize=True):
        ...
        lpips_loss = self.perceptual(target.contiguous(), pred.contiguous(), normalize=normalize)
