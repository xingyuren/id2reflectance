...
class FFHQPBRMIXDataset(data.Dataset):

    def __init__(self, opt):
        super(FFHQPBRMIXDataset, self).__init__()
        ...
        if self.io_backend_opt['type'] == 'lmdb':
            ...
            self.paths = paths_from_folder(opt['datapath_pbrdomain'])
            self.paths_aux = paths_from_folder(opt['datapath_ffhq'])
            assert len(self.paths) <= len(self.paths_aux)
    ...
    def __getitem__(self, index):
        ...
        if index % 2 == 0:
            ...
            random.shuffle(self.paths_aux)
            gt_path = self.paths_aux[index]
