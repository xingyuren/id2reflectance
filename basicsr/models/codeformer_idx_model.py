    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.opt['rank'] == 0:
            if (current_iter + 1) % self.sample_iter == 0:
                ...
                with torch.no_grad():
                    imgs = list()
                    input_img = ((self.input.cpu()) * 0.5 + 0.5).numpy()
                    for r in range(self.b):
                        imgs.append(input_img[r, ...])
                    
                    img_fake, _, _ = self.net_g_ema(self.input, w=0)
                    ...
                    plot_batch(imgs, osp.join(self.opt['path']['visualization'], 'EMA_iter_' + str(current_iter + 1) + '.jpg'))
